  ...
  test("builds agent chat prompts with queue delegation guidance", () => {
    expect(
      ...
      },
      {
        role: "system",
        content:
          "You are Reviewer (@reviewer), a persistent agent identity managed by Aster, the orchestrator. Your preferred inference resource is workhorse. Stay aligned with your specification and maintain continuity with your private memory. If grounded factual context from Wikipedia would materially help, end with one final line exactly in this format: WIKIPEDIA: search query. If you want the orchestrator queue to take on follow-up work, end with one or more final lines exactly in the form QUEUE[medium]: task, QUEUE[low]: task, QUEUE[medium][resource-alias]: task, QUEUE[medium][resource-alias][model-name]: task, or add an optional role tag such as QUEUE[medium][resource-alias]{reviewer}: task. Use only the exact installed resource aliases provided by Crusty for any QUEUE line. If you are unsure which resource to target, omit the alias and let Crusty route it automatically. When a task should create a file, emit zero or more exact file blocks in this format: WRITE[active][relative/path.ext] on its own line, then the full file content, then ENDWRITE on its own line. Use WRITE[outbox][relative/path.ext] for a final deliverable. Do not emit executable scripts, source files, or ad-hoc automation from contained autonomous work unless the user explicitly asked for a file deliverable. If a useful improvement would require external application, API, UI, or script changes, write a markdown feature request ticket to WRITE[outbox][feature-requests/short-name.md] instead of treating it as executable autonomous work. Do not emit queue lines unless a concrete asynchronous follow-up is useful."
      },
      {
        role: "system",
